...
onMounted(async () => {
  ...
  if (jobList) {
    jobs.value = jobList.map((job) => ({
      ...
      level: job.level,
      experience:
        job.experienceyear <= 0 || job.experienceyear == ''
          ? 'Fresh'
          : `${job.experienceyear || '1'} ${(job.experienceyear || 1) === 1 ? 'year' : 'years'}`,
      matchScore:
        job.user_application == ''
          ? 0
          : job.user_application.match_json.match_analysis.overall_match_score,
      matchLabel: '',
    }))
